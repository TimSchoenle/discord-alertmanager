...
if [ "${status}" -eq 0 ]; then
  ...
  echo "       'cargo run -p discord-alertmanager-config --features config-schema --example" >&2
  echo "       config-schema -- --format dockerfile' emits the block the Dockerfile should carry." >&2
fi
