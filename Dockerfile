...
# Global Build Args
ARG BINARY_NAME=discord-alertmanager
ARG USER_ID=1001
ARG GROUP_ID=1001
ARG EXECUTION_DIRECTORY=/app

FROM lukemathwalker/cargo-chef:latest-rust-alpine@sha256:917b051d1fc8e234a3aad123378b5263c95fa5d8739439ee25aa789c2db97a90 AS chef-base
...
# Debug-symbol upload is entirely opt-in (see the sentry-cli RUN in `builder` below), but the CLI
# is fetched here unconditionally: it is a few megabytes, the layer is shared by every build of
# every architecture, and skipping it would mean deciding *now*, at FROM time, whether a later
# `--secret id=sentry_token` will be supplied.
...
RUN --mount=type=cache,target=/usr/local/cargo/registry,id=cargo-registry-${TARGETARCH} \
    ...
    cargo build --release --target ${BUILD_TARGET} -p discord-alertmanager && \
...
#
# `FROM chef` rather than `FROM builder`: `discord-alertmanager-config`'s `config-schema` example
# reads the same source tree and wants the same toolchain, but nothing it emits depends on the
# release binary having linked. Basing it on `builder` would queue the contract behind a
# fat-LTO musl build that takes minutes longer, and would drag the host-side export in CI along
# with that whole build.
#
...
# the field, which is what keeps the committed copy in `docs/config.json` stable across a version
# bump and so keeps the drift gate from failing every release pull request.
RUN --mount=type=cache,target=/usr/local/cargo/registry,id=cargo-registry-contract-${TARGETARCH} \
    ...
    sh -eu -c '\
      ...
      cargo run --quiet -p discord-alertmanager-config --features config-schema --example config-schema \
...
# Emitted verbatim by:
#
#   cargo run -p discord-alertmanager-config --features config-schema --example config-schema -- --format labels
#
...
# the whole file passes an image whose block was pasted twice, and cutting by line count passes
# one that lost a label.
...
LABEL dev.terrace.config.contract.version="1" \
      ...
      dev.terrace.config.prefix="DAM_"
...
# Documentation, not a rule: `ingest.bind` decides the real address and defaults to
# `0.0.0.0:9099`. Stated so `docker inspect` answers the question without the reader opening
# config.example.toml, and so a compose file that forgets `ports:` still gets a mapping from
# `docker run -P`.
EXPOSE 9099
