-- Card-scoped outbox coalescing. The SQLite backend carries the same number and filename, so the
-- two are diffable side by side.
...
ALTER TABLE outbox ADD COLUMN notification_id BIGINT;
